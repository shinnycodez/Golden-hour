...
const Header = () => {
  ...
  const adminEmails = ["mueezimran1@gmail.com", "maheenomar98@gmail.com"];

  return (
    <>
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#F2DCC5] bg-[#F2DCC5] px-4 md:px-10 py-3 relative">
        <div className="flex items-center gap-4 md:gap-8">
          <Link to="/" className="flex items-center gap-2 md:gap-4 text-[#141414]">
<h2
  className="text-[#141414] text-lg md:text-xl font-bold leading-tight tracking-[-0.015em]"
  style={{ fontFamily: "'Playfair Display', serif" }}
>
  GOLDEN HOUR <br className="hidden lg:inline" /> BY MAHEEN
</h2>
          ...
          <div className="hidden md:flex items-center gap-5 lg:gap-9">
            ...
            <Link to="/products?category=charms" className="text-[#141414] text-sm font-medium  transition"> charms ⋆｡𖦹°</Link>
            <Link to="/products?category=Earings" className="text-[#141414] text-sm font-medium  transition">Earings ˚⋆𐙚｡</Link>
...
#F2DCC5] items-center justify-center pl-4 rounded-l-lg border-r-0">
...
#F2DCC5] focus:border-none h-full placeholder:text-neutral-500 px-4 rounded-l-none border-l-0 pl-2 text-base font-normal leading-normal"
                value={searchQuery}
...
#F2DCC5] text-[#141414] hover:bg-[#FFDCDC] transition">🛒 Cart</button>
            {user ? (
              <>
                <button onClick={handleLogout} className="px-4 h-10 rounded-lg bg-[#
#F2DCC5] text-[#141414] text-sm font-semibold hover:bg-[#
#F2DCC5] transition">Logout</button>
...
#F2DCC5] text-white text-sm font-semibold hover:opacity-90 transition"
              >
        ...
        {isMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-lg z-50 border-t border-[#ededed]">
            <div className="px-4 py-2 space-y-2">
              ...
              <Link to="/products?category=charms" onClick={() => setIsMenuOpen(false)} className="blocsk px-4 py-3 hover:bg-[#FFE9DD] rounded-lg transition"> charms ⋆｡𖦹°
</Link>
